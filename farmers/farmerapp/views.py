...
def cropdisease(request):
    ...
    if request.method == "POST" and 'image' in request.FILES:
        ...
        # Get the prediction result and suggestions
        ...
        # Create a new CropPrediction entry and save it to the database
        crop_prediction = CropPrediction(
            user=user,
            uploaded_image=uploaded_image,
            plant_type=result,  # The type of plant/tree predicted
            growth_tips="\n".join(growth_tips),  # Join the growth tips into a single text block
            care_tips="\n".join(care_tips),  # Join the care tips into a single text block
            pesticide_suggestions="\n".join(pesticide_suggestions)  # Join pesticide suggestions into a single text block
        )
